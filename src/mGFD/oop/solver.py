...
from mGFD.oop.pde import PDE, PoissonEquation, WaveEquation                                                             # PDE physics classes.
...
class Solver:                                                                                                           # Solver class definition.
    ...
    """
    def __init__(self, domain: Domain, pde: PDE, device: str = "cpu", cfl: float = 0.5,
                 implicit: bool = True, nvec: int = 15, upwind: Optional[bool] = None,
        ...
        Input:
            ...
            pde         PDE         PDE physics formulation (PoissonEquation, HeatEquation, WaveEquation, etc.).
            ...
            upwind      bool        Use directional upwind-biased stencils. Default is None (auto-selected).
    ...
    def solve(self, t_span: Tuple[float, float] = (0.0, 1.0), dt: Optional[float] = None) -> SolverResult:              # Main solve method.
        ...
        # 1. Stationary PDEs (Order 0)
        if self.pde.order == 0 or isinstance(self.pde, PoissonEquation):                                                # Check if stationary PDE.
            ...
            if self.device == "cpu":                                                                                    # Evaluate CPU backend.
                ...
                u_ap, vec, converged = solve_cpu(p, bc_callable, f_src, self.pde.operator, upwind_flag, vec, self.nvec, verbose=self.verbose)# Execute CPU solver.
            else:                                                                                                       # Evaluate CUDA backend.
                from mGFD.solvers._backends.cuda.stationary import solve_cuda                                           # Import CUDA backend solver.
                u_ap, vec, converged = solve_cuda(p, bc_callable, f_src, self.pde.operator, upwind_flag, vec, self.nvec, verbose=self.verbose)# Execute CUDA solver.
        ...
        # 2. First-Order Transient PDEs (Order 1: Heat / Advection-Diffusion)
        elif self.pde.order == 1:                                                                                       # Check if 1st-order transient PDE.
            if self.pde.coef is None:                                                                                   # Validate coefficients are set for type checker.
                raise ValueError("PDE coefficients are undefined.")                                                     # Raise ValueError exception.
            ...
            if dt is not None:                                                                                          # If explicit dt provided.
                ...
                t_use  = max(2, int(round(t_len / dt_use)) + 1)                                                         # Compute discrete time step points.
            else:                                                                                                       # Automatic CFL estimation.
                dt_use = dt_est                                                                                         # Use estimated dt.
                t_use  = t_est                                                                                          # Use estimated steps.

            lam = 0.5                                                                                                   # Crank-Nicolson theta parameter for implicit scheme.

            if self.device == "cpu":                                                                                    # Evaluate CPU backend.
                from mGFD.solvers._backends.cpu.time_derivative1 import solve_cpu                                       # Import CPU backend solver.
                u_ap, vec, converged = solve_cpu(p, None, t_use, self.pde.coef, self.pde.operator, upwind_flag, vec, self.nvec,# Call CPU solver.
                ...
                u_ap, vec, converged = solve_cuda(p, None, t_use, self.pde.coef, self.pde.operator, upwind_flag, vec, self.nvec,# Call CUDA solver.
                                                  self.implicit, lam, verbose=self.verbose, ic=self.pde.ic, bc=bc_callable,# Pass parameters.
                                                  source=self.pde.source, t_span=t_span)                                # Pass t_span.
        ...
        # 3. Second-Order Transient PDEs (Order 2: Wave Equation)
        elif self.pde.order == 2 or isinstance(self.pde, WaveEquation):                                                 # Check if 2nd-order transient PDE.
            if self.pde.coef is None:                                                                                   # Validate coefficients are set for type checker.
                raise ValueError("PDE coefficients are undefined.")                                                     # Raise ValueError exception.
            upwind_flag = True if self.upwind is None else self.upwind                                                  # Auto-select upwind directional stencils for wave.
            damping_val = getattr(self.pde, 'damping', 0.0)                                                             # Extract velocity damping parameter \eta.
            alpha_val   = getattr(self.pde, 'alpha', -0.1)                                                              # Extract HHT-\alpha dissipation parameter.
            t_len       = t_span[1] - t_span[0]                                                                         # Time domain length.
            ...
            lam = 0.25                                                                                                  # Newmark beta parameter for implicit scheme.

            if self.device == "cpu":                                                                                    # Evaluate CPU backend.
                from mGFD.solvers._backends.cpu.time_derivative2 import solve_cpu                                       # Import CPU backend solver.
                u_ap, vec, converged = solve_cpu(p, None, g_val, t_use, self.pde.coef, self.pde.operator, upwind_flag, vec, self.nvec,# Call CPU solver with correct positional args.
                ...
                u_ap, vec, converged = solve_cuda(p, None, g_val, t_use, self.pde.coef, self.pde.operator, upwind_flag, vec, self.nvec,# Call CUDA solver with correct positional args.
                                                  self.implicit, lam, verbose=self.verbose, ic=self.pde.ic, bc=bc_callable,# Pass implicit, lam, verbose, ic, bc.
                                                  source=self.pde.source, damping=damping_val, alpha=alpha_val, t_span=t_span)# Pass source, damping, alpha, t_span.
