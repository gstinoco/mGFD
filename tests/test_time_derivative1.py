...
def test_time_derivative1_heat() -> None:
    ...
    u_ap, vec = TimeDerivative1(p, f, t, [v], operator=L, implicit=True, linear_solver="spsolve", verbose=False)                        # Execute solver.
    ...
    u_ap_bicgstab, _ = TimeDerivative1(p, f, t, [v], operator=L, implicit=True, linear_solver="bicgstab", verbose=False)                # Execute solver.
    error_bicgstab = np.sqrt(np.mean((u_ap_bicgstab[:, -1] - u_ex[:, -1])**2))                                                          # Compute RMS error.
    assert error_bicgstab < 1e-1                                                                                                        # Verify error bound.

    # 4. Execution and Assertion - GMRES
    u_ap_gmres, _ = TimeDerivative1(p, f, t, [v], operator=L, implicit=True, linear_solver="gmres", verbose=False)                      # Execute solver.
    error_gmres = np.sqrt(np.mean((u_ap_gmres[:, -1] - u_ex[:, -1])**2))                                                                # Compute RMS error.
    assert error_gmres < 1e-1                                                                                                           # Verify error bound.
