...
def f(x: np.ndarray, y: np.ndarray, t_val: float, coef: List[float]) -> np.ndarray:
    """
    Initial displacement and analytical solution for the 2D Wave equation.

    Evaluates the exact wave displacement field:
        u(x, y, t) = cos(pi * c * sqrt(2) * t) * sin(pi * x) * sin(pi * y)
    ...
    Input:
        ...
        coef            1           list            Wave speed coefficient [c].

    Output:
        f_val           m           ndarray         Evaluated analytical displacement field.
    """
    c_val = coef[0]                                                                                                                     # Extract wave speed constant.
...
def g(x: np.ndarray, y: np.ndarray, t_val: float, coef: List[float]) -> np.ndarray:
    """
    Initial velocity and analytical time derivative for the 2D Wave equation.

    Evaluates the exact velocity field:
        g(x, y, t) = -pi * c * sqrt(2) * sin(pi * c * sqrt(2) * t) * sin(pi * x) * sin(pi * y)
    ...
    Output:
        g_val           m           ndarray         Evaluated analytical velocity field.
    """
    c_val = coef[0]                                                                                                                     # Extract wave speed constant.
    omega = np.pi * c_val * np.sqrt(2)                                                                                                  # Temporal angular frequency.
    return -omega * np.sin(omega * t_val) * np.sin(np.pi * x) * np.sin(np.pi * y)                                                       # Return analytical velocity.
...
def process_cloud(dataset: str, scale: str, cloud_path: str, results_path: str, save: bool, verbose: bool = True, **kwargs: Any) -> None:
    ...
    dmp_param       = kwargs.get('damping', 0.0)                                                                                        # Extract optional damping factor.
    alpha_param     = kwargs.get('alpha', 0.0)                                                                                          # Extract optional HHT-alpha dissipation factor.
    ...
    pde         = PDE(operator=L, ic=ic_func, g=g, order=2, coef=[c])                                                                   # Formulate Wave PDE.
    pde.damping = dmp_param                                                                                                             # Attach custom damping parameter.
    pde.alpha   = alpha_param                                                                                                           # Attach custom HHT-alpha parameter.
    solver      = Solver(domain, pde, device=device, cfl=cfl_param, nvec=nvec, lam=lam_param, verbose=verbose_solvers, implicit=True)   # Create Solver.
    ...
    metrics['Damping']    = dmp_param                                                                                                   # Store damping parameter.
    metrics['Alpha']      = alpha_param                                                                                                 # Store HHT-alpha parameter.
