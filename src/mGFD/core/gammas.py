    ...
    for i in np.arange(m):                                                                              # Loop over nodes.
        if p[i, 2] == 0:                                                                                # Interior node: compute stencil weights.
            ...
            try:                                                                                        # 1) Try NNLS for strictly positive weights
                G_trace = float(np.trace(M @ M.T))
                lam = (1e-12 + reg_factor * G_trace) if G_trace > 0.0 else 1e-12
                A_aug = np.vstack([M, np.sqrt(lam) * np.eye(nvec_i)])
                b_aug = np.concatenate([np.array(L).reshape(-1), np.zeros(nvec_i)])
                YY, _ = nnls(A_aug, b_aug)
            ...
            if YY is None or (not np.all(np.isfinite(YY))):                                             # 2) Fallback to standard Mínimos Cuadrados
                try:
                    G   = M @ M.T
                    tr  = float(np.trace(G))
                    reg = (1e-12 + reg_factor * tr) if tr > 0.0 else 1e-12
                    c   = np.linalg.solve(G + reg * np.eye(5), L)
                    YY  = (M.T @ c).reshape(-1)
                except Exception:
                    YY = None

            if YY is None or (not np.all(np.isfinite(YY))):                                             # 3) Absolute fallback to pseudoinverse
                YY = (np.linalg.pinv(M) @ L).reshape(-1)
...
def compute_sparse_matrix(p, vec, L):
    """
