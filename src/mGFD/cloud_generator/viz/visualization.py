...
matplotlib.use('Agg') # Set non-interactive backend for file generation             # Force headless backend.
...
def create_visualization(points: np.ndarray, regions_list: list[int], output_base: str, classifications: list[str] = None) -> bool:
    ...
    Input:
        points              ndarray         Array of (x, y) coordinates for all points.
        regions_list        list            Region assignments for each node.
        output_base         str             Base path for output files (without extension).
        classifications     list            Node classifications ('boundary' or 'interior'). Default is None.
    ...
    try:                                                                            # Start plotting process.
        ...
        logging.info(f"Visualization saved to {png_file} and {svg_file}")           # Log successful save.
        return True                                                                 # Return success state.
        
    except Exception as e:                                                          # If error occurs during plotting.
        logging.error(f"Error creating visualization: {e}")                         # Log exception details.
        try:                                                                        # Attempt cleanup.
...
def render_neighbors_graph(points: np.ndarray, neighbors_indices: np.ndarray, regions: list[int], output_base: str) -> bool:
    ...
    Input:
        points              ndarray         Array of (x, y) coordinates for all nodes.
        neighbors_indices   ndarray         NxK array of neighbor indices. -1 indicates no neighbor.
        regions             list            Array of region identifiers for coloring.
        output_base         str             Base path for output files (without extension).
    ...
    try:                                                                            # Start graph plotting process.
        ...
        logging.info(f"Graph visualization saved to {png_file} and {svg_file}")     # Log success.
        return True                                                                 # Return success state.
        
    except Exception as e:                                                          # If error occurs during rendering.
        logging.error(f"Error creating graph visualization: {e}")                   # Log the exact error.
        try:                                                                        # Attempt cleanup.
