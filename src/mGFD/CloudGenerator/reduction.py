...
def reduce_points_by_region_single(input_csv: str, output_csv: str) -> list[dict] | None:
    """
    ...
    Output:
        reduced_rows    list        List of dictionaries representing the reduced points if successful,
    ...
    try:                                                                            # Try to execute the main block.
        ...
        for region_id in sorted_regions:                                            # Iterate over the sorted region IDs.
            ...
            if region_id == 1 or str(region_id) == '1':                             # Check if the region is the main region (1).
                # Preserve all boundary points, reduce only interior points
                ...
                # Reduce boundary points softly: keep 3, delete 1 (25% reduction per pass)
                ...
                # Reduce interior points aggressively: take every 2nd point (50% reduction per pass)
                reduced_interiors = interiors[::2]                                  # Take every second point from the interiors.
                
                # Add boundaries, then the reduced interior
...
def reduce_points_by_region_multiple(input_csv: str, output_csv: str, multiplier: int = 2) -> list[dict] | None:
    """
    ...
    Output:
        result          list        Final list of dictionaries with reduced points if successful,
                                    None if any error occurred.
    """
    try:                                                                            # Try to execute the main block.
...
def filter_main_region_points_in_subregions(rows: list[dict]) -> list[dict]:
    """
    ...
    Input:
        rows            list        List of dictionaries containing cloud data.
    
    Output:
        filtered_rows   list        Filtered list of dictionaries.
    """
    try:                                                                            # Try to execute the main block.
        ...
        for region_id in regions:                                                   # Iterate over all available regions.
            ...
            if len(region_points_array) >= 3:                                       # Check if there are enough points for a polygon.
                try:                                                                # Try to compute the convex hull.
                    ...
                    logging.warning(f"Failed to create convex hull for subregion {region_id}: {e}") # Log a warning.
        ...
        logging.error(f"Error filtering main region points: {e}")                   # Log an error message.
        return rows                                                                 # Return the original rows if filtering fails.

def reduce_points_by_region_with_filtering(input_csv: str, output_csv: str, multiplier: int = 2, filter_subregions: bool = False) -> list[dict] | None:
    """
    ...
    Input:
        ...
        filter_subregions   bool        Enable geometric filtering before reduction. Default is False.
    
    Output:
        result              list        Processed list of dictionaries with reduced and optionally filtered points,
                                        or None if an error occurred.
    """
    try:                                                                            # Try to execute the main block.
        if filter_subregions:                                                       # Check if subregion filtering is requested.
            # Apply subregion filtering first
            rows = []                                                               # Initialize the list for the rows.
            with open(input_csv, 'r', newline='', encoding='utf-8') as f:           # Open the input CSV for reading.
                reader = csv.DictReader(f)                                          # Create a DictReader for the file.
                for row in reader:                                                  # Iterate over the rows.
                    # Convert types
                    try:                                                            # Try to convert coordinates and region.
                        row['x'] = float(row['x'])                                  # Convert 'x' coordinate to float.
                        row['y'] = float(row['y'])                                  # Convert 'y' coordinate to float.
                        try:                                                        # Try to convert region.
                            row['region'] = int(float(row['region']))               # Convert 'region' to integer.
                        except:                                                     # If region conversion fails.
                            pass                                                    # Ignore and keep as is.
                        rows.append(row)                                            # Add the parsed row to the list.
                    except ValueError:                                              # If coordinate conversion fails.
                        continue                                                    # Skip to the next row.
                        
            filtered_rows = filter_main_region_points_in_subregions(rows)           # Filter the points in the main region.
            
            # Save filtered data to temporary file
            with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False, newline='', encoding='utf-8') as temp_file: # Create a temporary file.
                temp_filtered_file = temp_file.name                                 # Store the name of the temporary file.
                fieldnames = list(filtered_rows[0].keys()) if filtered_rows else ['x', 'y', 'region'] # Determine field names.
                writer = csv.DictWriter(temp_file, fieldnames=fieldnames)           # Create a DictWriter for the temporary file.
                writer.writeheader()                                                # Write the CSV header.
                writer.writerows(filtered_rows)                                     # Write the filtered data.
            
            # Apply reduction to filtered data
            if multiplier <= 1:                                                     # Check if the multiplier is 1 or less.
                result = reduce_points_by_region_single(temp_filtered_file, output_csv) # Apply single reduction pass.
            else:                                                                   # If the multiplier is greater than 1.
                result = reduce_points_by_region_multiple(temp_filtered_file, output_csv, multiplier) # Apply multiple passes.
            
            # Clean up temporary file
            try:                                                                    # Try to delete the temporary file.
                os.remove(temp_filtered_file)                                       # Remove the file from disk.
            except:                                                                 # If removal fails.
                pass                                                                # Skip and continue.
            
            return result                                                           # Return the reduction result.
        else:                                                                       # If filtering is not requested.
            # Standard reduction without filtering
            if multiplier <= 1:                                                     # Check if the multiplier is 1 or less.
                return reduce_points_by_region_single(input_csv, output_csv)        # Apply single reduction pass directly.
            else:                                                                   # If the multiplier is greater than 1.
                return reduce_points_by_region_multiple(input_csv, output_csv, multiplier) # Apply multiple passes directly.
                
    except Exception as e:                                                          # Handle general exceptions.
        logging.error(f"Error in reduce_points_by_region_with_filtering: {e}")      # Log an error message.
        return None                                                                 # Return None on error.

def reduce_points_by_region(input_csv: str, output_csv: str, multiplier: int = 2) -> list[dict] | None:
    """
    ...
    Output:
        result          list        Processed list of dictionaries with reduced points if successful,
                                    None if any error occurred.
    """
