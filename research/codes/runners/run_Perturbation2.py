...
Overview:
    ...
    but configures the stationary solver with upwind=False. It runs on each point cloud under Data/
...
Workflow:
    ...
    - Solve the stationary problem with Stationary(..., upwind=False)
...
vx: float  = 1.0                                                                                                                        # Advection velocity in x (used inside operator L only).
vy: float  = 0.0                                                                                                                        # Advection velocity in y (used inside operator L only).
...
def process_cloud(dataset: str, scale: str, cloud_path: str, results_path: str, save: bool, verbose: bool = True) -> None:              # Run one cloud case and write outputs to Results/.
    ...
    u_ap, vec  = Stationary(                                                                                                            # Solve the stationary perturbation problem.
        p, phi, f, operator = L, upwind = False, vec = vec0, nvec = NVEC, verbose = False                                               # Use cached neighbors, no upwinding.
    ...
    if save:                                                                                                                            # Save solution to VTK format if requested.
        ExportVTK.export_stationary_vtk(p, u_ap, u_ex, out_dir, basename="Perturbation2_Solution",
                                        cloud_path=cloud_path, verbose=verbose)                                                         # Save VTK data to disk.
        
        plot_stationary(p, u_ap, save=True, nom=os.path.join(out_dir, 'Perturbation2_Approximation'),
                                        title='Stationary Appx', verbose=verbose)                                                       # Save 3D scatter image.
        
        if scale == '5':                                                                                                                # Only for scale 5.
            plot_stationary(p, u_ex, save=True, nom=os.path.join(os.path.dirname(out_dir), 'Perturbation2_Exact'),
                        title='Theoretical Solution', verbose=verbose)                                                                  # Create independent plot of exact solution.

def main() -> None:
    """
    main
    Entry point for the Perturbation2 batch script.
