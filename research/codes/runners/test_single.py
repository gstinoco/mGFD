...
sys.path.append(BASE_DIR)                                                                                                       # Allow local imports.

from mGFD.io.io import load_points                                                                                              # Point cloud loading utility.
import research.benchmarks.benchmark as bmk                                                                                     # Benchmarking functions to test.
...
def test_single_cloud() -> None:
    ...
    """
    # 1. Load test cloud
    cloud_path: str = os.path.join(os.path.dirname(BASE_DIR), 'data', 'Titicaca', '1', 'Titicaca_cloud.csv')                                     # Path to sample cloud.
    if not os.path.exists(cloud_path):                                                                                          # Validate file existence.
        logger.error(f"No se encontró la nube de prueba en: {cloud_path}")                                                      # Report error.
        return                                                                                                                  # Abort test.

    logger.info(f"Iniciando prueba rápida en una única nube: {cloud_path}")                                                     # Report startup.
    p = load_points(cloud_path, verbose=False)                                                                                  # Load point cloud silently.
    logger.info(f"Nube cargada con {len(p)} puntos.")                                                                           # Report loaded point count.
    ...
    for name, func in tests:                                                                                                    # Iterate all defined tests.
        logger.info(f"\n[{name}] Ejecutando...")                                                                                # Report current test.
        try:                                                                                                                    # Wrap execution to catch solver crashes.
            res: Dict[str, Any] = func(p)                                                                                       # Execute benchmark function on cloud.
            logger.info(f"  [EXITO] Tiempo: {res['execution_time_seconds']:.3f}s | RMSE Promedio: {res['avg_numerical_error']:.3e}") # Report metrics.
        except Exception as e:                                                                                                  # Catch any exception.
            logger.error(f"  [ERROR] Falló la prueba {name}: {e}")                                                              # Report failure reason.
