...
def process_cloud(dataset, scale, cloud_path, results_path, save):                                      # Run one cloud case and write outputs to Results/.
    ...
    u_ap, u_ex, vec = TimeDerivative2(                                                                  # Solve Wave with implicit time stepping.
        p, f, g, t, [c], operator = L, implicit = True, lam = 0.5, vec = vec0, nvec = NVEC              # Solve with cached neighbors when available.
    ...
    if save:                                                                                            # Save solution to VTK format if requested.
        ...
        ExportVTK.export_transient_vtk(p, u_ap, u_ex, t, T, out_dir, basename="Wave_Solution", cloud_path=cloud_path) # Save VTK time series to disk.
