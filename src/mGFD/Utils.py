...
from mGFD.Neighbors import find_distances                                                            # Distance estimator.
...
def get_valid_triangulation(p, nom):
    '''
    ...
    Input:
        p                           ndarray         Point cloud with columns [x, y, ...].
        nom                         str             Base filename or path to identify the dataset and scale.
    
    Output:
        global_triangles            ndarray[int]    Array of shape (n_triangles, 3) containing valid Delaunay simplices, or None if failed.
    '''
