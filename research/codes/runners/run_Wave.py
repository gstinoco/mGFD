...
def f(x: np.ndarray, y: np.ndarray, t_val: float, coef: List[float]) -> np.ndarray:
    ...
    Input:
        ...
        t_val           1           float           Time at which to evaluate.
        coef            1           list            List with the wave propagation coefficient.

    Output:
        f_val           m           ndarray         Evaluated initial displacement.
    """
...
def g(x: np.ndarray, y: np.ndarray, t_val: float, coef: List[float]) -> np.ndarray:
    ...
    Output:
        g_val           m           ndarray         Evaluated initial velocity.
    """
...
def process_cloud(dataset: str, scale: str, cloud_path: str, results_path: str, save: bool, verbose: bool = True, **kwargs: Any) -> None:
    ...
    res_call = TimeDerivative2(                                                                                                         # Solve the transient wave problem (Callable).
        p, f, g, t, [c], operator = L, vec = vec0, nvec = nvec, implicit = True, lam = 1.0, linear_solver = linear_solver, device = device, matrix_free = matrix_free, preconditioner = preconditioner, verbose = verbose_solvers
    )                                                                                                                                   # Extract solver result object.
    
    # --- Precompute exact solution arrays for Array/Pandas tests and Metrics ---
    T_arr = np.linspace(0, 1, t)                                                                                                        # Reconstruct time vector.
    f_arr = np.zeros([len(p), t])                                                                                                       # Initialize exact solution matrix (displacement).
    for k in range(t):                                                                                                                  # Loop over all time steps.
        f_arr[:, k] = f(p[:, 0], p[:, 1], T_arr[k], [c])                                                                                # Compute exact theoretical displacement.
    ...
    res_arr = TimeDerivative2(                                                                                                          # Solve using array inputs.
        p, f_arr, g_arr, t, [c], operator = L, vec = vec0, nvec = nvec, implicit = True, lam = 1.0, linear_solver = linear_solver, device = device, matrix_free = matrix_free, preconditioner = preconditioner, verbose = False
    )
    ...
    res_pd = TimeDerivative2(                                                                                                           # Solve using Pandas inputs.
        p, f_pd, g_pd, t, [c], operator = L, vec = vec0, nvec = nvec, implicit = True, lam = 1.0, linear_solver = linear_solver, device = device, matrix_free = matrix_free, preconditioner = preconditioner, verbose = False
    )
