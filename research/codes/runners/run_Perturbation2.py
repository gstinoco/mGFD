...
def process_cloud(dataset: str, scale: str, cloud_path: str, results_path: str, save: bool, verbose: bool = True) -> None:      # Run one cloud case and write outputs to Results/.
    ...
    if save:                                                                                                                    # Save solution to VTK format if requested.
        ...
        plot_stationary(p, u_ap, save=True, nom=os.path.join(out_dir, 'Perturbation2_Approximation'), title='Stationary Appx', verbose=verbose) # Save 3D scatter image.
        
        if scale == '5':                                                                                                        # Only for scale 5.
            plot_stationary(p, u_ex, save=True, nom=os.path.join(out_dir, 'Perturbation2_Exact'), title='Theoretical Solution', verbose=verbose) # Create independent plot of exact solution.
