...
def process_cloud(dataset: str, scale: str, cloud_path: str, results_path: str, save: bool, verbose: bool = True) -> None:      # Run one cloud case and write outputs to Results/.
    ...
    u_ap, vec = TimeDerivative2(                                                                                                # Solve Wave with implicit time stepping.
        p, f, g, t, [c], operator = L, implicit = True, lam = 0.5, vec = vec0, nvec = NVEC, verbose = False                     # Solve with cached neighbors when available.
    ...
    if save:                                                                                                                    # Save solution to VTK format if requested.
        ExportVTK.export_transient_vtk(p, u_ap, u_ex, t, T_arr, out_dir, basename="Wave_Solution", cloud_path=cloud_path, verbose=verbose) # Save VTK time series to disk.
        plot_transient(p, u_ap, save=True, nom=os.path.join(out_dir, 'Wave_Approximation'), title='Transient Approximation', verbose=verbose) # Save transient animation.
