...
from mGFD import Cloud, Dirichlet, AdvectionDiffusion, Solver                                                           # OOP Architecture classes.
...
def process_cloud(dataset: str, scale: str, cloud_path: str, results_path: str, save: bool, verbose: bool = True, **kwargs: Any) -> None:
    ...
    if 'callable' in input_types:                                                                                       # Evaluate condition.
        ...
        pde      = AdvectionDiffusion(v=nu, v_x=a, v_y=b, ic=ic, source=source)                                         # Formulate PDE.
        solver   = Solver(domain, pde, device=device, cfl=cfl_param, nvec=nvec, upwind=upwind, verbose=verbose_solvers) # Create Solver.
        res_call = solver.solve(t_span=t_span, dt=( (t_span[1]-t_span[0]) / t_param ) if t_param else None)             # Solve PDE.
    ...
    if 'array' in input_types:                                                                                          # Evaluate condition.
        domain   = cloud.set_boundary(Dirichlet(u_ex))                                                                  # Set Dirichlet boundary.
        pde      = AdvectionDiffusion(v=nu, v_x=a, v_y=b, ic=u_ex[:, 0], source=source)                                 # Formulate PDE.
        solver   = Solver(domain, pde, device=device, cfl=cfl_param, nvec=nvec, upwind=upwind, verbose=False)           # Create Solver.
        res_arr  = solver.solve(t_span=t_span, dt=((t_span[1]-t_span[0]) / t_used))                                     # Solve PDE.
    ...
    if 'pandas' in input_types:                                                                                         # Evaluate condition.
        ...
        domain = cloud.set_boundary(Dirichlet(bc_pd))                                                                   # Set Dirichlet boundary.
        pde    = AdvectionDiffusion(v=nu, v_x=a, v_y=b, ic=u_ex[:, 0], source=source)                                   # Formulate PDE.
        solver = Solver(domain, pde, device=device, cfl=cfl_param, nvec=nvec, upwind=upwind, verbose=False)             # Create Solver.
        res_pd = solver.solve(t_span=t_span, dt=((t_span[1]-t_span[0]) / t_used))                                       # Solve PDE.
        if u_ap is not None:                                                                                            # Evaluate condition.
