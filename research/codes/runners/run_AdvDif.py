...
def process_cloud(dataset: str, scale: str, cloud_path: str, results_path: str, save: bool, verbose: bool = True, **kwargs: Any) -> None:
    ...
    def f(x: np.ndarray, y: np.ndarray, t_val: float, coef: List[float]) -> np.ndarray:                                                 # Define dynamic initial condition with xc, yc.
        """
        f
        ...
        """
        return (1 / (4 * t_val + 1)) * np.exp(                                                                                          # Return output values.
            - (x - coef[1] * t_val - xc)**2 / (coef[0] * (4 * t_val + 1)) - (y - coef[2] * t_val - yc)**2 / (coef[0] * (4 * t_val + 1)) # Execute statement.
        )                                                                                                                               # Execute statement.
    ...
    if 'callable' in input_types:                                                                                                       # Check if callable test is enabled.
        res_call = TimeDerivative1(                                                                                                     # Solve the transient Advection-Diffusion problem (Callable).
            p, f, t=t_param, cfl=cfl_param, coef=[v, a, b], operator=L, vec=vec0, nvec=nvec, implicit=True, lam=0.5, upwind=upwind, device=device, verbose=verbose_solvers # Assign p, f, t.
    ...
    f_arr = (1 / (4 * T + 1)) * np.exp(- (X - a * T - 0.5)**2 / (v * (4 * T + 1)) - (Y - b * T - 0.5)**2 / (v * (4 * T + 1)))           # Vectorized computation of analytical solution matrix.
    ...
    if 'array' in input_types:                                                                                                          # Check if array test is enabled.
        res_arr = TimeDerivative1(                                                                                                      # Solve using array inputs.
            p, f_arr, t=t_used, coef=[v, a, b], operator=L, vec=vec0, nvec=nvec, implicit=True, lam=0.5, upwind=upwind, device=device, verbose=False # Assign p, f_arr, t.
        )                                                                                                                               # Execute statement.
    ...
    if 'pandas' in input_types:                                                                                                         # Check if pandas test is enabled.
        ...
        res_pd = TimeDerivative1(                                                                                                       # Solve using Pandas inputs.
            p, f_pd, t=t_used, coef=[v, a, b], operator=L, vec=vec0, nvec=nvec, implicit=True, lam=0.5, upwind=upwind, device=device, verbose=False # Assign p, f_pd, t.
        )                                                                                                                               # Execute statement.
        if u_ap is not None:                                                                                                            # If previous result exists, validate.
