...
for region, files in regions.items():
    ...
    if '_p.csv' in files and '_tt.csv' in files:
        ...
        Graph.Cloud_Stationary(p, tt, u_ap, u_ex, save = True, nom = nom)
        ...
        Graph.Cloud_Transient(p, tt, u_ap, u_ex, save = True, nom = nom)
        ...
        # Advection Equation
        print('Advection Problem')
        ## Problem Parameters
        a    = 0.3
        b    = 0.2
        t    = 2000                                                                      # Number of time-steps.
        Save = False                                                                     # Should I save the results?
        f = lambda x, y, t, coef: 0.2*np.exp((-(x-.5-coef[0]*t)**2-(y-.5-coef[1]*t)**2)/.001)

        ## Operator
        L = np.vstack([[-a], [-b], [0], [0], [0], [0]])                                  # L = [D, E, A, B, C, F] from operator Lu = Au_xx + Bu_xy + Cu_yy + Du_x + Eu_y + F_u

        ## Problem Solving
        u_ap, u_ex, vec = TimeDerivative1(p, f, t, [a, b], operator = L, triangulation = False, tt = [], implicit = False, lam = 0.5)

        ##Compute the Error
        er = Errors.Cloud_Transient(p, vec, u_ap, u_ex)

        ## Save the Error and the solution
        nom = os.path.join(results_path, 'Advection', region, 'Error.txt')
        with open(nom, 'w') as file:
            file.write(str(np.mean(er)))

        nom = os.path.join(results_path, 'Advection', region, 'Computed Solution.csv')
        np.savetxt(nom, u_ap, delimiter=',', fmt='%d')
        
        nom = os.path.join(results_path, 'Advection', region, 'Theoretical Solution.csv')
        np.savetxt(nom, u_ex, delimiter=',', fmt='%d')

        ## Plot the solution
        nom = os.path.join(results_path, 'Advection', region, 'Solution.mp4')
        Graph.Cloud_Transient(p, tt, u_ap, u_ex, save = True, nom = nom)

        nom = os.path.join(results_path, 'Advection', region, 'Solution')
        Graph.Cloud_Transient_Steps(p, tt, u_ap, u_ex, nom = nom)
