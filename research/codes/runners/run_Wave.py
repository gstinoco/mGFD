...
def process_cloud(dataset: str, scale: str, cloud_path: str, results_path: str, save: bool, verbose: bool = True, **kwargs: Any) -> None:
    ...
    res_call = TimeDerivative2(                                                                                                         # Solve the transient wave problem (Callable).
        p, f, g, t, [c], operator = L, vec = vec0, nvec = nvec, implicit = True, lam = 1.0, linear_solver = linear_solver, verbose = verbose_solvers
    ...
    res_arr = TimeDerivative2(                                                                                                          # Solve using array inputs.
        p, f_arr, g_arr, t, [c], operator = L, vec = vec0, nvec = nvec, implicit = True, lam = 1.0, linear_solver = linear_solver, verbose = False
    )
    ...
    res_pd = TimeDerivative2(                                                                                                           # Solve using Pandas inputs.
        p, f_pd, g_pd, t, [c], operator = L, vec = vec0, nvec = nvec, implicit = True, lam = 1.0, linear_solver = linear_solver, verbose = False
    )
