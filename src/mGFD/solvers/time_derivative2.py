...
def TimeDerivative2(p: Union[np.ndarray, Any], 
    ...
    if device == "cpu":
        ...
        u_ap, vec, converged = solve_cpu(p, f, g, t, coef, operator, upwind, vec, nvec, implicit, lam, linear_solver, preconditioner, matrix_free, verbose)
    else:
        from mGFD.solvers._backends.cuda.time_derivative2 import solve_cuda
        u_ap, vec, converged = solve_cuda(p, f, g, t, coef, operator, upwind, vec, nvec, implicit, lam, linear_solver, preconditioner, matrix_free, verbose)
