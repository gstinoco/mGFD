...
def f(x: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    """
    return -(1e-5) * (2 * np.exp((x - 1) / (1e-5)) - 2 * x**2 + y * (np.exp((x - 1) / (1e-5)) / (1e-5)**2 - 2) * (y - 1)) - y * (2 * x - np.exp((x - 1) / (1e-5)) / (1e-5)) * (y - 1)# Return output values.
...
def process_cloud(dataset: str, scale: str, cloud_path: str, results_path: str, save: bool, verbose: bool = True, **kwargs: Any) -> None:
    ...
    L    = np.vstack([[vx], [vy], [-D], [0], [-D], [0]])                                                                # Operator coefficients for Au_xx + Bu_xy + Cu_yy + D    # 3. Solver Execution via OOP Interface
    ...
    if 'callable' in input_types:                                                                                       # Check if callable test is enabled.
        ...
        pde      = PoissonEquation(source=f, operator=L)                                                                # Formulate Stationary PDE with L.
        solver   = Solver(domain, pde, device=device, nvec=nvec, upwind=upwind, verbose=verbose_solvers)                # Create Solver.
    ...
    if 'array' in input_types:                                                                                          # Check if array test is enabled.
        ...
        pde      = PoissonEquation(source=f_arr, operator=L)                                                            # Formulate Stationary PDE with L.
        solver   = Solver(domain, pde, device=device, nvec=nvec, upwind=upwind, verbose=False)                          # Create Solver.
    ...
    if 'pandas' in input_types:                                                                                         # Check if pandas test is enabled.
        ...
        pde    = PoissonEquation(source=f_pd, operator=L)                                                               # Formulate Stationary PDE with L.
        solver = Solver(domain, pde, device=device, nvec=nvec, upwind=upwind, verbose=False)                            # Create Solver.
        res_pd = solver.solve()                                                                                         # Solve PDE.  # Execute statement.
        if u_ap is not None:                                                                                            # If previous result exists, validate.
