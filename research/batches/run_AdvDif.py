...
def process_cloud(dataset, scale, cloud_path, results_path, save, verbose=True):                      # Run one cloud case and write outputs to Results/.
    ...
    # Calculate pulse center dynamically inside the interior region
    interior = p[p[:, 2] == 0]
    if len(interior) > 0:
        x_mean, y_mean = np.mean(p[:,0]), np.mean(p[:,1])
        dists = (interior[:,0] - x_mean)**2 + (interior[:,1] - y_mean)**2
        idx = np.argmin(dists)
        xc, yc = interior[idx, 0], interior[idx, 1]
    else:
        xc, yc = np.mean(p[:,0]), np.mean(p[:,1])
        
    f = lambda x, y, t, coef: (1 / (4 * t + 1)) * np.exp(
        - (x - coef[1] * t - xc)**2 / (coef[0] * (4 * t + 1)) - (y - coef[2] * t - yc)**2 / (coef[0] * (4 * t + 1))
    )
    ...
    u_ap, vec = TimeDerivative1(                                                                        # Solve Advection-Diffusion with implicit time stepping.
        p, f, t, [v, a, b], operator = L, implicit = True, lam = 0.5, upwind = True, vec = vec0, nvec = NVEC, verbose = verbose
    ...
    if save:                                                                                            # Save solution to VTK format if requested.
        T = np.linspace(0, 1, t)                                                                        # Reconstruct time vector.
        ExportVTK.export_transient_vtk(p, u_ap, u_ex, t, T, out_dir, basename="AdvDif_Solution", cloud_path=cloud_path, verbose=verbose) # Save VTK time series to disk.
        
        plot_transient(p, u_ap, save=True, nom=os.path.join(out_dir, 'AdvDif_Approximation'), title='Transient Approximation', verbose=verbose)
