...
def process_cloud(dataset: str, scale: str, cloud_path: str, results_path: str, save: bool, verbose: bool = True) -> None:      # Run one cloud case and write outputs to Results/.
    ...
    # 2. Data Loading & Dynamic Problem Definition
    ...
    def f(x: np.ndarray, y: np.ndarray, t_val: float, coef: List[float]) -> np.ndarray:                                         # Define dynamic initial condition with xc, yc.
        ...
        return (1 / (4 * t_val + 1)) * np.exp(
            - (x - coef[1] * t_val - xc)**2 / (coef[0] * (4 * t_val + 1)) - (y - coef[2] * t_val - yc)**2 / (coef[0] * (4 * t_val + 1))
        )
    ...
    u_ap, vec = TimeDerivative1(                                                                                                # Solve Advection-Diffusion with implicit time stepping.
        p, f, t, [v, a, b], operator = L, implicit = True, lam = 0.5, upwind = True, vec = vec0, nvec = NVEC, verbose = False   # Solve with cached neighbors when available.
    ...
    if save:                                                                                                                    # Save solution to VTK format if requested.
        ExportVTK.export_transient_vtk(p, u_ap, u_ex, t, T_arr, out_dir, basename="AdvDif_Solution", cloud_path=cloud_path, verbose=verbose) # Save VTK time series to disk.
        plot_transient(p, u_ap, save=True, nom=os.path.join(out_dir, 'AdvDif_Approximation'), title='Transient Approximation', verbose=verbose) # Save transient animation.
