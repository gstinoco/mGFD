...
def RHS(p, boun_n, inne_n, phi, f):
    ...
    Input:
        p           m x 3           array-like      Point cloud [x, y, flag].
        boun_n      m               array-like[bool] Boundary mask (True on boundary nodes).
        inne_n      m               array-like[bool] Interior mask (True on interior nodes).
        phi                         function        Boundary function phi(x, y).
        f                           function        Forcing function f(x, y).
...
def CloudStencil(p, vec, L, reg_factor=1e-8):
    """
    CloudStencil
    ...
    Input:
        p           m x 3           array-like      Point cloud [x, y, flag].
        vec         m x nvec        array-like[int] Neighbor indices per node (padded with -1).
        L           5 x 1           array-like      Operator coefficients [D, E, A, B, C].
    ...
    for i in np.arange(m):                                                                              # Loop over nodes.
        if p[i, 2] == 0:                                                                                # Interior node: compute stencil weights.
            ...
            try:                                                                                        # Prefer a stable 5x5 solve on normal equations.
                ...
                reg = (1e-12 + reg_factor * tr) if tr > 0.0 else 1e-12                                  # Diagonal regularization magnitude.
                ...
                YY = None                                                                               # Keep YY unset to force pseudoinverse fallback.
            if YY is None or (not np.all(np.isfinite(YY))):                                             # Fallback when solve fails or returns NaNs/Infs.
                YY = (np.linalg.pinv(M) @ L).reshape(-1)                                                # Pseudoinverse-based weights.
...
def ApplyCloudStencil(u, vec, diag, w):
    """
    ApplyCloudStencil
    ...
    Input:
        u           m               array-like      Values at nodes.
        vec         m x nvec        array-like[int] Neighbor indices per node (padded with -1).
        diag        m               array-like      Central weights.
        w           m x nvec        array-like      Neighbor weights aligned with vec.
...
def BiCGStab(matvec, b, x0=None, tol=1e-10, max_iter=2000):
    """
    BiCGStab
    ...
    Input:
        ...
        x0                          array-like|None Optional initial guess.
    ...
    try:                                                                                                # Prefer SciPy when available (faster/robust).
        ...
        try:                                                                                            # Old SciPy API uses tol.
            x, info = bicgstab(A, b_np, x0 = x0_np, tol = tol, maxiter = max_iter)                      # Attempt SciPy BiCGStab (old signature).
